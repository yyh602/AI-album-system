...
try {
    ...
    // 方法 1：使用 Neon 官方連線字串（包含 endpoint 參數）
    ...
    // 方法 2：使用 pgsql DSN 格式，包含 endpoint 參數
    echo "<h3>方法 2：pgsql DSN 格式</h3>";
    $dsn2 = "pgsql:host=$host;port=$db_port;dbname=$dbname;sslmode=require;options=endpoint%3Dep-sweet-band-a1bt630p;user=$db_user;password=$db_pass";
    echo "DSN: $dsn2<br>";
    ...
    // 方法 3：使用原生 pgsql 函數，包含 endpoint 參數
    echo "<h3>方法 3：原生 pgsql 函數</h3>";
    try {
        $connection_string = "host=$host port=$db_port dbname=$dbname user=$db_user password=$db_pass sslmode=require options='endpoint=ep-sweet-band-a1bt630p'";
